...
```{r 2}
head(rawData)
length(unique(rawData$concentration) )
```
...
ggplot( pharmacoData, aes( log2(pharmacoData$viability) ) ) +
    geom_histogram(fill = "white", colour="black") +
    facet_wrap(~pharmacoData$doseID)
```
...
library(ggplot2)
  ggplot(cellLinesSummary,aes(x=auc_GDSC, y=auc_CCLE)) +
    geom_point(cex=0.5) +
    xlab("GDSC AUC") +
    ylab("CCLE AUC") + facet_wrap(facets=~drug)
```
...
cor.coef <- round(cor(cellLinesSummary$auc_GDSC, cellLinesSummary$auc_CCLE),2)
ggplot(data=cellLinesSummary, aes(x=x,y=y)) +
  geom_point() +
